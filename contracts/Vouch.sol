...
contract Vouch {
    ...
    function generateVouchHash(address recipient, string memory message) public view returns (bytes32 hash) {
        hash = keccak256(abi.encode(address(this), VOUCH_ID, recipient, message));
    }
    ...
    function _vouch(address recipient, string calldata message, address sender) private returns (bool success) {
        bytes32 hashed = generateVouchHash(recipient,message);
